from solar_irradiance import aoi, get_total_irradiance
from solar_position import get_solarposition
...
class GTI_class:
    def __init__(self,_year,_city,path_test):
    ...
    def process_gti(self):
        ...
        return poa_global
def GTI_results(city_DES,path_test):
    editable_data_path =os.path.join(path_test, 'editable_values.csv')
    editable_data = pd.read_csv(editable_data_path, header=None, index_col=0, squeeze=True).to_dict()[1]
    city = city_DES
    global folder_path
    ...
    for year in range(int(editable_data['starting_year']),int(editable_data['ending_year'])+1):
        ...
        weather_year = GTI_class(year,city,path_test)
