...
class TwoStageOpt(Problem):
    ...
    def evaluate_operation(self,_electricity_EF,_electricity_demand,_heating_demand,_A_swept,_A_solar,_CAP_CHP_elec,_CAP_boiler,_CAP_battery):
        ...
        if _electricity_demand==0:
            Cost_minimzed = boilers.NG_boiler(heating_demand/boilers.NG_boiler(0,CAP_boiler,self.path_test)[4],CAP_boiler,self.path_test)[2] #$
            emission_objective = boilers.NG_boiler(heating_demand/boilers.NG_boiler(0,CAP_boiler,self.path_test)[4],CAP_boiler,self.path_test)[3] #kg CO2
            return 'Cost ($)', Cost_minimzed,'Emissions (kg CO2)', emission_objective ,'CHP',0,'Boiler', heating_demand/boilers.NG_boiler(0,CAP_boiler,self.path_test)[4],'Grid',0
        else:
            if self.use_boilers == 'yes':
                F_F_boiler = 1
                model.F_boiler = pyo.Var(bounds=(0,CAP_boiler/boilers.NG_boiler(0,CAP_boiler,self.path_test)[4])) #Decision space for natural gas fuel rate
            ...
            if self.use_CHP=='yes':
                F_F_CHP = 1
                model.F_CHP = pyo.Var(bounds=(0,CHP_system.CHP(CAP_CHP_elect,0,self.path_test)[5])) #Decision space for CHP fuel rate
            ...
            if self.use_grid =='yes':
                ...
                F_E_grid = 0
                grid_model = 0
            model.OBJ_cost = pyo.Objective(expr = CHP_system.CHP(CAP_CHP_elect,CHP_model,self.path_test)[3]*F_F_CHP +boilers.NG_boiler(Boiler_model,CAP_boiler,self.path_test)[2]*F_F_CHP + grid_model*self.electricity_prices*self.UPV_elect*F_E_grid) #$
            model.Constraint_elect = pyo.Constraint(expr = CHP_system.CHP(CAP_CHP_elect,CHP_model,self.path_test)[0]*F_F_CHP + grid_model*F_E_grid - _electricity_demand>=0) # Electricity balance of demand and supply sides
            model.Constraint_heat = pyo.Constraint(expr = boilers.NG_boiler(Boiler_model,CAP_boiler,self.path_test)[0]*F_F_boiler + CHP_system.CHP(CAP_CHP_elect,CHP_model,self.path_test)[1]*F_F_CHP -heating_demand>=0) # Heating balance of demand and supply sides
            ...
            if len(results.solution)==0:
                ...
                if self.use_grid =='yes':
                    ...
                    value_grid_model = 0
                Cost_minimzed =CHP_system.CHP(CAP_CHP_elect,value_CHP_model*F_F_CHP,self.path_test)[3]+ boilers.NG_boiler(value_Boiler_model*F_F_boiler,CAP_boiler,self.path_test)[2]+value_grid_model*self.electricity_prices*F_E_grid
                emission_objective = CHP_system.CHP(CAP_CHP_elect,value_CHP_model*F_F_CHP,self.path_test)[4] + boilers.NG_boiler(value_Boiler_model*F_F_boiler,CAP_boiler,self.path_test)[3] +value_grid_model*_electricity_EF*F_E_grid #kg CO2
