...
import solar_PV
import wind_turbine
path_parent = Path(sys.path[0])
editable_data_path =os.path.join(path_parent.parent.absolute(), 'EditableFile.csv')
editable_data = pd.read_csv(editable_data_path, header=None, index_col=0, squeeze=True).to_dict()[1]
components_path = os.path.join(sys.path[0])
...
def battery(electricity_demand_bat,hour,E_bat_,_A_solar,_A_swept,_CAP_battery,G_T_now,V_wind_now):
    ...
    E_bat = E_bat_
    renewables_elect =  solar_PV.solar_pv(A_solar, hour,0,G_T_now,1)[0] + wind_turbine.wind_turbine(A_swept, hour,0,V_wind_now,1)[0]
