...
class TwoStageOpt(Problem):
    ...
    def operating_cost(self,_A_solar,_A_swept,_CAP_CHP_elect,_CAP_boiler,_CAP_battery):
        ...
        for represent in range(num_clusters):
            ...
            for hour in range(0,24):
                ...
                if hour==0:
                    E_bat[hour]=0
                battery_results = battery.battery(electricity_demand[hour],hour,E_bat[hour],A_solar,A_swept,CAP_battery,G_T_now,V_wind_now)
    ...
    def evaluate_operation(self,_electricity_EF,_electricity_demand,_heating_demand,_A_swept,_A_solar,_CAP_CHP_elec,_CAP_boiler,_CAP_battery):
        ...
        if _electricity_demand==0:
            Cost_minimzed = boilers.NG_boiler(heating_demand/boilers.NG_boiler(0,CAP_boiler)[4],CAP_boiler)[2] #$
            emission_objective = boilers.NG_boiler(heating_demand/boilers.NG_boiler(0,CAP_boiler)[4],CAP_boiler)[3] #kg CO2
            return 'Cost ($)', Cost_minimzed,'Emissions (kg CO2)', emission_objective ,'CHP',0,'Boiler', heating_demand/boilers.NG_boiler(0,CAP_boiler)[4],'Grid',0
        else:
            if use_boilers == 'yes':
                F_F_boiler = 1
